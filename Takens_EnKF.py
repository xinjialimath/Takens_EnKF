...
class TakensEnsembleKalmanFilter(object):
    """
    This is rewrited the ensemble Kalman filter (EnKF).
    The raw is filterpy.kalman import EnsembleKalmanFilter
    Parameters
    ----------
    ...
    def __init__(self, x, P, dim_z, dt, N, d):
        self.sigmas = None
        if dim_z <= 0:
        ...
        self.sigmas_h = np.zeros((self.N, 1))
        self.k = 5
    ...
    def initialize(self, x, P):
        ...
        for index in range(self.N):
            train_states = load_value(os.path.join('../Lorenz_data/{}/data'.format(index), 'states.npy'))
    ...
    def update(self, z, R=None):
        ...
        z_mean = np.mean(self.sigmas_h, axis=0)

        P_zz = (outer_product_sum(self.sigmas_h - z_mean) / (N-1)) + R
        P_xz = outer_product_sum(
                self.sigmas - self.x, self.sigmas_h - z_mean) / (N - 1)
        ...
        for i in range(N):
            self.sigmas[i] += dot(self.K, z + e_r[i] - self.sigmas_h[i])
